/**
 * @file
 * Provides UI/UX progressive enhancements on webtheme's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.webtheme.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-webtheme-custom-color', changedInput.value);
    inputToSync.setAttribute('data-webtheme-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-webtheme-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-webtheme-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes webtheme theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-webtheme-custom-color',
      textInput.getAttribute('data-webtheme-custom-color'),
    );
  ...
  /**
   * webtheme Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.webthemeColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'webtheme-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'webtheme-color-picker',
        '[data-drupal-selector="webtheme-color-picker"] input[type="text"]',
      );
